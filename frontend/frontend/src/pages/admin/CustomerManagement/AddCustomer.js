  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data === true) {
        ...
        console.log("Thêm khách hàng thất bại. Vui lòng thử lại.");
      }
    } catch (err) {
      console.log(err);
    }
    ...
    <>
      <div className=" container mx-auto p-4 w-full border rounded-lg relative text-2xl">
        ...
        <form className="mt-6" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className=" absolute right-4 bottom-4 bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition"
          >
            Thêm mới
          </button>
        </form>
