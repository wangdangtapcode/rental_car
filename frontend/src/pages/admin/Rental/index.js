...
function ContractRentalPage() {
  ...
  const handleConfirmContract = useCallback(async () => {
    ...
    const updateVehicleFromConTract = vehiclesWithNotes.map((item) => ({
      id: item.vehicle.id,
      vehicleCondition: item.conditionNotes,
      status: "RENTED",
    }));
    const createContractVehicleDetails = vehiclesWithNotes.map((item) => ({
      id: item.vehicle.id,
      rentalPrice: item.vehicle.rentalPrice,
      penalties: null,
    }));
    const createCollateral = collaterals.map((desc) => ({ description: desc }));
    const creatRentalContract = {
      customerId: customer.id,
      employeeId: 123,
      startDate: searchParams.startDate,
      endDate: searchParams.endDate,
      contractVehicleDetails: createContractVehicleDetails,
      depositAmount: depositAmount,
      collaterals: createCollateral,
      status: "ACTIVE",
    };
    console.log(updateVehicleFromConTract);
    console.log(createContractVehicleDetails);
    console.log(createCollateral);
    console.log(creatRentalContract);
